...
(function(jQuery) {
	...
	this.each(function() {
		...
		// -------- he invalidado el slidenum colocandole una s delante de la palabra ------------------------------------------------------------------------------
		var f = 1;
		jQuery(this).find('dt').each(function(){
			jQuery(this).css({'width':dtHeight,'top':dtTop,'margin-left':dtOffset});	
			if(settings.sslideNum == true){
				jQuery('<span class="sslide-number">'+0+f+'</span>').appendTo(this);
				if(jQuery.browser.msie){	
					var sslideNumLeft = parseInt(jQuery(this).find('.sslide-number').css('left')) - 14;
					jQuery(this).find('.sslide-number').css({'left': sslideNumLeft})
					if(jQuery.browser.version == 6.0 || jQuery.browser.version == 7.0){
						jQuery(this).find('.sslide-number').css({'bottom':'auto'});
					}
					if(jQuery.browser.version == 8.0){
					var sslideNumTop = jQuery(this).find('.sslide-number').css('bottom');
					var sslideNumTopVal = parseInt(sslideNumTop) + parseInt(jQuery(this).css('padding-top'))  - 12; 
					jQuery(this).find('.sslide-number').css({'bottom': sslideNumTopVal}); 
					}
				} else {
					var sslideNumTop = jQuery(this).find('.sslide-number').css('bottom');
					var sslideNumTopVal = parseInt(sslideNumTop) + parseInt(jQuery(this).css('padding-top')); 
					jQuery(this).find('.sslide-number').css({'bottom': sslideNumTopVal}); 
				}
			}
			f = f + 1;
